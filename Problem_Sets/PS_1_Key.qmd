...
- Update your installation of R if it is not currently the most recent version: [CRAN](https://cran.r-project.org/index.html)
- Install **RTools for Windows** (Download R for Windows -> Rtools).
    - Choose the version of Rtools that matches your R version (e.g. Rtools45 for R 4.5)
    - By default, Rtools will install into `C:\`. This is ideal. If you do not have root access to your laptop (i.e., if you are on a University controlled machine) you can install to your user directory. You might need to change the PATH variable in this case (if you need help with that, let us know).
...
- The first time you load `cmdstanr` with `library()`, it will prompt you to install the cmdstan backend (which does the actual compilation and sampling of models)
...
# does not matter.
d <- list(pass = c(rep(0, times = 62), rep(1, times = 9)))
...
Compiling the model might fail immediately with an error something like this:
...
Exit status: 2
Stderr:
make: *** /c/Users/.../OneDrive: Is a directory.  Stop.
```

This problem seems only to be present on Windows machines where you home directory is stored on OneDrive. The solution is to force `ulam()` to use `rstan` instead of `cmdstan`. Add the following code to the chunk above:
...
If your machinery does not work even after the fix above, let us know right away.
...
A frequentist analysis instead asks: for different possible parameters, what is the probability of observing this sample? The estimate is the parameter that leads to the highest probability of observing the sample. The measure of uncertainty (a confidence interval) is based in this same framework: assuming a true population parameter, what would you observe with repeated sampling?
...
- Place these values along with an index value of 1 in a tibble (`index = 1`). We will use the index to organize the CIs in the next step. Plot the confidence interval with a point for the mean and a horizontal segment at the index value for the CI. Add a vertical line showing the true proportion in the population. Set the x-axis to range from 0 to 1.
...
> The frequentist CI makes the assumption of repeated "sampling", which effectively means re-running an experiment over and over. This is very unlikely to happen in any real-world situation (although it works great in a computer). The Bayesian HPDI has no such assumptions. We can directly interpret a Bayesian HPDI as the relative probability of parameter estimates given the priors, model, and data.
