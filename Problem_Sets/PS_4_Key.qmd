...
HT |>
  group_by(Mismatch) |>
  summarize(mean_Survival = mean(Survival), sd_Survival = sd(Survival))
...
get_prior(logSurvival ~ Mismatch - 1, data = HT)
...
get_prior(Calcium ~ Sex_Trt - 1, data = BP)
...
PP_long |>
  ...
  summarize(mean_Calcium = mean(Calcium), var_Calcium = var(Calcium))
...
MM <- readxl::read_excel("../Data/Milk.xlsx") |>
  ...
  drop_na() |>
  rename(Milk_energy = kcal.per.g, Fat = perc.fat, Lactose = perc.lactose)
```
